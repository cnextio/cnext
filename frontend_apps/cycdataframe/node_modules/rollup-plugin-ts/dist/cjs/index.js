...
class ResolveCache {
    ...
    get(options) {
        ...
        // Otherwise, proceed
            ...
            resolvedModule.resolvedFileName = path__default["default"].normalize(ensureAbsolute(cwd, resolvedModule.resolvedFileName));
            if (resolvedModule.resolvedFileName.endsWith(D_TS_EXTENSION)) {
                ...
                if (isTslib(id)) {
                    const candidate = path__default["default"].normalize(setExtension(resolvedModule.resolvedAmbientFileName, `.es6${JS_EXTENSION}`));
...
function replaceBabelHelpers(code, filename, target) {
    const matches = target === "cjs" ? [
        ...stringutil.matchAll(code, BABEL_REQUIRE_RUNTIME_HELPER_ESM_REGEXP_1),
        ...stringutil.matchAll(code, BABEL_REQUIRE_RUNTIME_HELPER_ESM_REGEXP_2)
    ] : [
...
class CompilerHost extends ModuleResolutionHost {
    ...
    getSourceFile(fileName, languageVersion = this.getScriptTarget()) {
        var _a;
        const absoluteFileName = isTypeScriptLib(fileName) ? path__default["default"].join(this.getDefaultLibLocation(), fileName) : ensureAbsolute(this.getCwd(), fileName);
...
function typescriptRollupPlugin(pluginInputOptions = {}) {
    ...
    const filter = (id) => internalFilter(id) || internalFilter(path__default["default"].normalize(id)) || internalFilter(path__default["default"].native.normalize(id));
    /**
    ...
    return {
        ...
        async renderChunk(code, chunk, outputOptions) {
            ...
            const transpilationResult = await core.transformAsync(updatedCode, Object.assign(Object.assign(Object.assign({}, config), { filenameRelative: ensureRelative(cwd, chunk.fileName) }), (updatedMap == null
                ? {}
                : {
        ...
        async transform(code, file) {
            ...
            if (sourceDescription == null) {
                ...
                // Otherwise, pass it on to Babel to perform the rest of the transpilation steps
                    ...
                    const transpilationResult = await core.transformAsync(sourceDescription.code, Object.assign(Object.assign({}, babelConfigResult.config), { filenameRelative: ensureRelative(cwd, normalizedFile), inputSourceMap: typeof sourceDescription.map === "string" ? JSON.parse(sourceDescription.map) : sourceDescription.map }));
                    if (transpilationResult == null || transpilationResult.code == null) {
                        return sourceDescription;
                    }
                    // Return the results
         ...
         */
        load(id) {
            const normalizedId = path__default["default"].normalize(id);
