...
import { setExecutorStatus, updateExecutorRestartCounter } from "../../../redux/reducers/ExecutorManagerRedux";
        ...
        socket.on(WebAppEndpoint.ExecutorManager, (result: string) => {
            ...
            if (!message.error) {
                if (message.command_name === ExecutorManagerCommand.get_status) {
                    ...
                    message.command_name === ExecutorManagerCommand.restart_kernel 
                ) {
        ...
        if (executorStatus?.resource_usage?.limits?.memory.rss > 0) {
            return (
                <>
                    ...
                    {(executorStatus?.resource_usage?.limits?.memory?.rss / 1024 ** 3).toFixed(3)} (GB)
                </>
            );
        } else {
