...
const createMonacoSelectionFromRelativeSelection = (editor, type, relSel, doc) => {
  ...
  if (start !== null && end !== null && start.type === type && end.type === type) {
    ...
    return (getMonaco()).Selection.createWithDirection(startPos.lineNumber, startPos.column, endPos.lineNumber, endPos.column, relSel.direction)
  }
  return null
}
          ...
          awareness.getStates().forEach((state, clientID) => {
            if (clientID !== this.doc.clientID && state.selection != null && state.selection.anchor != null && state.selection.head != null) {
              ...
              if (anchorAbs !== null && headAbs !== null && anchorAbs.type === ytext && headAbs.type === ytext) {
                ...
                newDecorations.push({
                  range: new (getMonaco()).Range(start.lineNumber, start.column, end.lineNumber, end.column),
                  options: {
      ...
      this.mux(() => {
        ...
        event.delta.forEach(op => {
          if (op.retain !== undefined) {
            ...
            const range = new (getMonaco()).Selection(pos.lineNumber, pos.column, pos.lineNumber, pos.column)
            ...
            const range = new (getMonaco()).Selection(pos.lineNumber, pos.column, endPos.lineNumber, endPos.column)
      ...
      editors.forEach(editor => {
        editor.onDidChangeCursorSelection(() => {
          if (editor.getModel() === monacoModel) {
            ...
            if (sel.getDirection() === (getMonaco()).SelectionDirection.RTL) {
              const tmp = anchor
              anchor = head
              head = tmp
            }
