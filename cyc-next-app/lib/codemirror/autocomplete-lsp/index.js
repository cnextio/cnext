...
class LanguageServerPlugin {
    ...
    async initialize({ documentText }) {
        try {
            const { capabilities } = await this.requestServer('initialize', {
                capabilities: {
                    textDocument: {
                        hover: {
    ...
    async requestHoverTooltip(view, { line, character }) {
        try{
            ...
            this.sendChange({ documentText: view.state.doc.toString() });
            const result = await this.requestServer('textDocument/hover', {
    ...
    _getDFCompletion_CodeEditor(context, line, character) {        
        ...
        // console.log(tree.cursor(curPos, 1).toString());
        let result = this._matchColNameExpression_CodeEditor(text, tree, curPos, this.colNamePattern_CodeEditor);   
        let items = null;     
        if (result.matched){
    ...
    _matchColValueExpression_DFFilter(text, tree, pos, matchPattern){        
        ...
        if (cursor.name == 'ColumnValueExpression'){
            ...
            console.log('_matchColumnValueExpression_DFFilter: match ColumnValueExpression, now search for column name');                                
            while(true){
                console.log(cursor.name);
                ...
                if (nextMatch==null){
                    ...
                    console.log(`_matchColumnValueExpression_DFFilter got a match with colName=${colName}`);
                    break;                    
                } else {
                    cursor[nextMatch]();                
                }          
        ...
        return {matched: matched, df_name: dfName, col_name: colName, str_content: strContent};
    }
    ...
    async requestCompletion_CodeEditor(context, { line, character }, { triggerKind, triggerCharacter, }) {
        try{
            ...
            if (this.transport.connection.readyState != WebSocket.CLOSED){      
                ...
                if (context.matchBefore(/[\w]+$/)){
                    ...
                    result = await this.requestServer('textDocument/completion', {
                        textDocument: { uri: this.documentUri },
                        position: { line, character },
                        context: {
                            triggerKind,
     ...
     */
    async requestCompletion_DFFilter(context, { line, character }, { triggerKind, triggerCharacter, }) {
        try{
            ...
            override: [
                async (context) => {
                    ...
                    if (trigKind === CompletionTriggerKind.Invoked &&
                        !context.matchBefore(/[\w'"]+$/)) { //added match for ' and "
                        return null;
                    }
                    return await plugin.requestCompletion_CodeEditor(context, offsetToPos(state.doc, pos), {
        ...
        autocompletion({
            override: [
                async (context) => {
                    ...
                    return await plugin.requestCompletion_DFFilter(context, offsetToPos(state.doc, pos), {
                        triggerKind: trigKind,
                        triggerCharacter: trigChar,
                    });
                },
