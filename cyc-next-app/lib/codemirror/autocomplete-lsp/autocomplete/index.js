...
class ActiveResult extends ActiveSource {
  ...
  handleUserEvent(tr, type, conf) {
    ...
    if (
      this.span &&
      (from == to || this.span.test(tr.state.sliceDoc(from, to)))
    )
...
function moveCompletionSelection(forward, by = 'option') {
  return (view) => {
    ...
    if (
      by == 'page' &&
      (tooltip = view.dom.querySelector('.cm-tooltip-autocomplete'))
    )
      step = Math.max(
        2,
        Math.floor(tooltip.offsetHeight / tooltip.firstChild.offsetHeight)
      );
