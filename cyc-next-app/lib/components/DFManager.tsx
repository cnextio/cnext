...
const DFManager = () => {
    ...
    const _sendPlotColumnHistogram = (df_id: string, col_name: string) => {
    ...
    const _getPlotHistogram = (df_id: string, col_list: []) => {
        for(var i=0; i<col_list.length; i++){
            const col_name = col_list[i];
            _sendPlotColumnHistogram(df_id, col_name);
        }
    }
    ...
    const _handleGetTableData = (message: {}) => {
        ...
        if(showHistogram && loadColumnHistogram){                
            if (dfUpdates['update_type'] == UpdateType.add_cols){                
                ...
                _getPlotHistogram(df_id, newColumns); 
                ...
                _getPlotHistogram(df_id, tableData['column_names']);
                _sendGetCountna(df_id);             
