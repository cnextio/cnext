...
const ExperimentManager = (props: any) => {
  ...
  function getSelectedRunningRun(state: DefaultRootState) {
    ...
    return runDict
      ? Object.keys(runDict).filter(
          (key) =>
            runDict[key]["_status"] == "RUNNING" && runDict[key]["selected"]
        )[0]
      : null;
  }
  ...
  function handleRunsChange(event: React.SyntheticEvent) {
    ...
    if (runDict) {
      dispatch(
        setRunSelection({ id: event.target.id, selected: event.target.checked })
      );
    }
  }
      ...
      <ExperimentLeftPanel>
        ...
        <ExpSelectorForm>
          <ExpSelector
            ...
            value={
              selectedExpId && expDict ? expDict[selectedExpId]["_name"] : null
            }
        ...
        <RunSelectorForm>
          {runDict &&
            Object.keys(runDict)
              .sort(
                (k1, k2) =>
                  runDict[k2]["_start_time"] - runDict[k1]["_start_time"]
              )
              .map((key, index) => (
                <RunSelectorLabel
                  key={key}
